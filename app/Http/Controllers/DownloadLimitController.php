...
class DownloadLimitController extends Controller
{
     ...
     */
    public function listAccountsAndDownloads()
    {
        // Lấy danh sách user
        $users = User::all();
    ...
    public function show($userId)
    {
        ...
        $downloadLimits = DownloadLimit::where('user_id', $userId)->get();

        // Kiểm tra xem có dữ liệu lượt tải hay không
        // if ($downloadLimits->isEmpty()) {
        //     // Nếu không có dữ liệu, bạn có thể xử lý theo ý của mình, ví dụ: hiển thị thông báo không có dữ liệu
        //     return view('content.downloadlimits.empty');
        // }

        // Nếu có dữ liệu, trả về view hiển thị thông tin chi tiết của lượt tải
        return view('content.downloadlimits.show', compact('downloadLimits'));
    }
